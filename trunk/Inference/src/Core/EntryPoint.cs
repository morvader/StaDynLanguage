                ...
                if (args.Length > 0) {
                    ...
                    for (int i = 0; i < args.Length; i++) {
                        if ((File.Exists(args[i])) || (Directory.Exists(args[i])))
                            initApp.LoadFile(new FileInfo(args[i]), directories);
                        else {
                    ...
                    initApp.Run(directories, null, Application.StartupPath+"\\Tests\\",
                        "ilasm.exe", "TypeTable.txt", TargetPlatform.CLR, false);
                }
                else
