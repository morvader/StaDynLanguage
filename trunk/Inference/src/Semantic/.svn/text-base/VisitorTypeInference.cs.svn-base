...
namespace Semantic {
    ...
    /// Implements Visitor pattern [Concrete Visitor].
    /// </remarks>
    public class VisitorTypeInference : VisitorAdapter {
        ...
        public override Object Visit(ReturnStatement node, Object obj) {
            ...
            // * If the formal return type has type variables, 
            if (methodVisited.Return.HasTypeVariables())
                // * The actual type is collected as a union type
        ...
        public override Object Visit(RelationalExpression node, Object obj) {
            ...
            // Check if the type can realize the operation
            if ((node.FirstOperand.ExpressionType != null) && (node.SecondOperand.ExpressionType != null))
                node.ExpressionType = (TypeExpression)node.FirstOperand.ExpressionType.AcceptOperation(new     RelationalOperation(node.SecondOperand.ExpressionType, node.Operator, this.getMethodAnalyzed(), true, node.Location), null);
        ...
        private TypeExpression abstractInterpretation(TypeExpression memberTypeExpression, TypeExpression[] arguments, AstNode node) {
            ...
            if (this.methodsVisited.Contains(md))
